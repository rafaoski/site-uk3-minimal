...
function ukNav($items, $options = array()) {
	...
	$defaults = array(
		...
		'maxNote' => setting('max-note'), // note shown at bottom of list when maxItems reached
	...
	foreach($itemsArray as $n => $item) {
		...
		if(empty($options['markup'])) {
			...
			$out .= "<a class='$linkClass' href='$item->url'>$itemLabel</a>";
		} else {
		...
		if($options['maxItems'] && $n+1 >= $options['maxItems'] && strlen($options['maxNote'])) {
			...
			if($options['maxLink']) $o = "<a class='uk-button uk-button-text uk-text-left uk-text-primary' href='{$item->parent->url}'>$o</a>";
...
function ukBlogPost(Page $page, $options = array()) {
	...
	$name = $page->createdUser->nick_name;
	$body = $page->get('body');
